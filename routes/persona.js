...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Persona created successfully', persona: normalizeBudgetFields(data) });
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    Object.entries(textFields).forEach(([from, to]) => { if (from in req.body) patch[to] = req.body[from] ?? null; });
    ...
    res.status(200).json({ success: true, message: 'Persona updated successfully', persona: normalizeBudgetFields(data) });
  } catch (error) {
    console.error('Update persona error:', error);
    res.status(500).json({ success: false, message: 'Internal server error' });
  }
...
router.post('/:id/avatar', upload.single('file'), async (req, res) => {
  try {
    ...
    if (!owns) return res.status(404).json({ success: false, message: 'Persona not found' });

    // object path and upload
    const objectPath = buildAvatarPath(req.user.id, personaId, req.file.mimetype, req.file.originalname);

    let uploadRes;
    const admin = getAdminSupabase();
    if (admin) {
      uploadRes = await admin.storage.from(AVATAR_BUCKET).upload(objectPath.replace(/^.*?\//, ''), req.file.buffer, {
        contentType: req.file.mimetype || 'application/octet-stream',
        upsert: true
      });
    } else {
      uploadRes = await req.supabase.storage.from(AVATAR_BUCKET).upload(objectPath.replace(/^.*?\//, ''), req.file.buffer, {
        contentType: req.file.mimetype || 'application/octet-stream',
        upsert: true
      });
    }
    ...
    if (uploadRes.error) {
      return res.status(400).json({ success: false, message: uploadRes.error.message });
    }
    ...
    if (dbErr) return res.status(400).json({ success: false, message: dbErr.message });

    // Optional: signed URL for preview (short lived)
    let signedUrl = null;
    const signer = admin || req.supabase; // prefer admin
    if (signer?.storage?.from) {
      const signed = await signer.storage.from(AVATAR_BUCKET).createSignedUrl(objectPath.replace(/^.*?\//, ''), 60 * 10);
      if (!signed.error) signedUrl = signed.signedUrl || signed.data?.signedUrl || null;
    }

    return res.json({ success: true, avatar: row, previewUrl: signedUrl, path: objectPath });
  } catch (e) {
...
router.get('/:id/avatar', async (req, res) => {
  try {
    ...
    if (error || !row) return res.status(404).json({ success: false, message: 'Avatar not found' });

    let signedUrl = null;
    const admin = getAdminSupabase();
    const signer = admin || req.supabase;
    if (signer?.storage?.from) {
      const signed = await signer.storage.from(AVATAR_BUCKET).createSignedUrl(row.object_path.replace(/^.*?\//, ''), 60 * 10);
      if (!signed.error) signedUrl = signed.signedUrl || signed.data?.signedUrl || null;
    }
