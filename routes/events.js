...
async function ensurePersonaOwnership(req, res, next) {
  try {
    ...
    if (!rawId) return res.status(400).json({ success: false, message: 'persona_id is required' });
    ...
    if (error || !persona) {
      return res.status(404).json({ success: false, message: 'Persona not found or not accessible' });
    }
    next();
  } catch (e) {
    return res.status(500).json({ success: false, message: 'Persona ownership check failed' });
  }
}
...
router.post('/personas/:id/events', ensurePersonaOwnership, async (req, res) => {
  try {
    const personaId = normalizeId(req.params.id);
    const { title, description, event_type, occurred_at, source, metadata, is_active } = req.body || {};
...
router.put('/events/:eid', async (req, res) => {
  try {
    ...
    if (typeof patch.metadata === 'string') {
      try { patch.metadata = JSON.parse(patch.metadata); } catch { delete patch.metadata; }
    }
